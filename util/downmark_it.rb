...
# 
# =Note
# This code is forked from https://github.com/cousine/downmark_it , and ported it from hpricot to nokogiri, and adjusted a little bit by utensil.github.com
#
...
module DownmarkIt
  ...
  def self.to_markdown(html, encoding = nil)
    raw = Nokogiri::HTML(html.gsub(/(\n|\r|\t)/, ""), nil, encoding)

    # headers
    (raw/"h1, h2, h3, h4, h5, h6").each do |header|
    ...
    return raw.to_s.gsub(/<!--.*-->/, '').sub(/<html>/, '').sub(/<\/html>/, '').sub(/<!DOCTYPE[^>]*>/, '')
  end
